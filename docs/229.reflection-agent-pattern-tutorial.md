...
**档位：** 主线篇（Part 3）
**本文边界：** 前驱篇 [228](228.plan-and-execute-agent-tutorial.md) 计划驱动。本篇 Generator/Reflector 自检。不讲无限自改。接续 [230](230.task-decomposition-agent-tutorial.md)。
**动手路径：** ① 读 Generator/Reflector 环 → ② 运行 `demo_reflection()` → ③ 看迭代如何改输出

### 要不要读
- **建议读：** 对输出质量敏感（报告、代码、对外文案）
- **可先跳过：** 低延迟聊天、简单工具调用
- **何时回来读：** 发现模型「一次生成」错误率高时
### 核心术语（本篇首次出现）
...
通俗说：专门挑 Generator 输出里的错误与遗漏。
**Reflection**（反思模式）：生成器产出草稿、反思器挑错后再改一版。
通俗说：写完让编辑挑错再改。
**Generator**（生成器）：产出初稿的组件，与 Reflector 配对。
通俗说：先写一版的作者。
---
...
对照上图：Reflection 用 Reflector 挑 Generator 的错——适合质量敏感、可自检的输出。
...
对照上图：Generator 产出草稿、Reflector 挑错再改——适合对外文案/代码等质量敏感输出。
```text
没有 Reflection 时：
...
对照上图：Reflection 环——230 任务分解处理复杂 Goal。
