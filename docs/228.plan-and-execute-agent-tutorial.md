...
**档位：** 主线篇（Part 3）
**本文边界：** 前驱篇 [227](227.react-agent-pattern-tutorial.md) ReAct。本篇 Plan-and-Execute。不讲 MCTS/动态重规划。接续 [229](229.reflection-agent-pattern-tutorial.md)。
**动手路径：** ① 读 Plan vs ReAct 表 → ② 运行 `demo_plan_execute()` → ③ 试 §先错对对

### 要不要读
- **建议读：** 复杂多步任务、需要显式计划再执行时
- **可先跳过：** 已用 ReAct 满足需求、任务步骤 ≤3 且固定
- **何时回来读：** ReAct 出现反复试错、token 浪费明显时
...
对照上图：Plan-and-Execute 先路线图后开车——适合步骤多、试错成本高的任务。
...
对照上图：先 Plan 再 Execute——步骤多的任务比 ReAct 少绕圈，但计划过时需重规划。
```text
ReAct 的局限：
...
对照上图：Plan vs ReAct——229 Reflection 补质量环。
