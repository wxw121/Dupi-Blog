...
**档位：** 地基篇（Part 1）
**本文边界：** 前驱篇 [214–216](214.rag-to-agent-transition-tutorial.md) 边界与选型。本篇五层企业架构总览。不讲单层完整实现。接续 [218](218.tool-calling-basics-tutorial.md) 工具层。
**动手路径：** ① 扫五层架构图 → ② 画自己系统分层草图 → ③ 按 §下一步 进入 218 工具层
...
对照上图：企业 Agent 五层——工具与权限在底层，编排与记忆在中层，观测与治理贯穿全程。
