...
**档位：** 主线篇（Part 4）
**本文边界：** 前驱篇 [234](234.long-term-agent-memory-tutorial.md) + [220](220.tool-parameter-validation-tutorial.md) 校验。本篇写入策略与 PII 过滤。合规见 [237](237.memory-privacy-deletion-tutorial.md)。接续 [236](236.memory-retrieval-policy-tutorial.md)。
**动手路径：** ① 读写入策略表 → ② 运行 `demo_write_policy()` → ③ 对照 237 删除
