...
先把代码高亮放到 RAG 前端链路里看：它不是独立页面，而是 Markdown 渲染管道里的一个 `code` 节点替换。


![代码高亮位置](../image/code-highlight-rag/01-highlight-pipeline.png)
```mermaid
flowchart LR
    A["LLM 输出 Markdown"] --> B["react-markdown 解析 AST"]
    B --> C["code 节点进入 CodeBlock"]
    C --> D["hljs/shiki 生成 token 样式"]
    D --> E["消息气泡展示"]
    E --> F["复制按钮/行号/主题"]
    A --> G["流式 delta"]
    G --> C
...
从图里应得出的结论是：安全清洗仍属于 Markdown 管道，高亮只负责让可信的代码文本更容易读。
...
选型可以用下面这张决策图快速判断。初学者不要先纠结“哪个更高级”，先看当前产品最需要速度、主题一致性还是流式体验。


![选型对比](../image/code-highlight-rag/02-hljs-vs-shiki.png)
```mermaid
flowchart TD
    A["需要代码高亮"] --> B{"是否强依赖 VS Code 级主题一致性?"}
    B -- "是" --> C["优先 shiki"]
    B -- "否" --> D{"是否要边流式边显示?"}
    D -- "是" --> E["优先 highlight.js 或 done 后再 shiki"]
    D -- "否" --> F["两者都可，PoC 选更快接入的"]
    C --> G["注意包体积与异步缓存"]
    E --> H["注意语言注册与性能"]
```

这张图的结论是：PoC 默认选 highlight.js 更稳；对外演示强调主题质感时再评估 shiki。
...
代码高亮不是单纯美化，它帮助读者区分语言、关键字、字符串和注释。对 RAG 答案页来说，还要保证 Markdown 渲染、语法高亮、行号和复制按钮互不破坏。
...
下面这张概念图把本篇几个职责放在一起：解析、清洗、高亮、复制和流式策略必须分层处理。


![概念地图](../image/code-highlight-rag/03-concept-map.png)
```mermaid
mindmap
  root((RAG 代码高亮))
    输入
...
这张图的结论是：代码高亮只是展示层增强，不能替代安全清洗，也不能证明代码真的可运行。
...
代码高亮上线前，用这组检查避免把“看起来更漂亮”变成安全或性能问题：

- Markdown 仍先经过 sanitize，高亮器不能直接处理未清洗 HTML。
- 未标语言的代码块有兜底样式，不应抛异常。
- 流式输出时未闭合围栏不反复全量高亮。
- 复制按钮复制的是纯文本，不是带 `<span>` 的 HTML。
- 深色和浅色主题都通过基本对比度检查。

## 15. 上线 FAQ 补充

**为什么不能直接把模型输出塞进 `innerHTML`？** 因为模型输出属于用户可控内容，可能包含恶意 HTML。高亮可以使用内部生成的 HTML，但原始 Markdown 必须先走安全清洗。

**highlight.js 和 shiki 能混用吗？** 可以，但不建议初期混用。先选一个路径跑通组件和测试，后续再按主题或性能需求迁移。

**流式时要不要每个字都高亮？** 不要。正在生成时可以显示普通 `<pre>`，等围栏闭合或消息 done 后再高亮。

**代码高亮能证明代码正确吗？** 不能。高亮只提升阅读和复制体验，代码是否可运行仍要靠原文、测试或沙箱。

## 16. 收束与下一步

RAG 答案里的代码块不是装饰，它影响运维、研发和审计读答案的效率。初学者掌握三个点即可：高亮插在 Markdown 渲染管道的 `code` 节点上；PoC 默认用 highlight.js 更省事；安全清洗永远在高亮之前。

下一步继续读 [174 流式打字机](174.streaming-typewriter-ui-tutorial（front-end）.md)，把“静态 Markdown 可读”推进到“答案流式出现时也稳定可读”。
