...
**档位：** 地基篇（Part 1）
**本文边界：** 前驱篇 [215](215.agent-vs-rag-vs-workflow-tutorial.md) 已能选型。本篇何时不该上 Agent。不讲实现细节。接续 [217](217.enterprise-agent-architecture-overview-tutorial.md)。
**动手路径：** ① 读 §决策树 → ② 标出自己项目里「不该用 Agent」的信号 → ③ 对照 §常见失败模式

### 核心术语（本篇首次出现）
**Agent**（智能体）：自主多步执行任务的系统。
通俗说：给目标后能自己规划并动手，不是只答一问。
...
对照上图：决策树入口是「步骤是否固定、是否需工具写外部世界」——满足 Workflow/RAG 就别硬上 Agent。
...
对照上图：不该用 Agent 的信号清单——与 215 选型表配合使用。
