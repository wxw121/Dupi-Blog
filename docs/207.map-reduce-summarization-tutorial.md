...
2. 或 metadata `chunk_type` + 查询时 **boost**（[88 metadata filter](88.metadata-filter-retrieval-tutorial.md)）；  
...
Map 段彼此 **无状态**，可 `asyncio` / 线程池并行——注意 **API 速率限制**（[69 Embedding 重试篇](69.embedding-retry-rate-limit-tutorial.md) 的限流思路同样适用 LLM）。  
...
若段内必含 **表格**，用 [64 HTML DOM](64.html-dom-chunking-tutorial.md) 或 [37 PDF 表格](37.pdf-layout-tables-tutorial.md) 保证 **表头与首行数据同段**，否则 Map 摘要会写「本段含表格（细节略）」——Reduce 阶段永远补不回来。
...
单段 Map **连续失败** → 该段用 **原文截断占位** 并标 `map_degraded`；全文档失败率 **>10%** **中止发布** 摘要索引，避免 **空洞总览** 误导业务（[184 看板](184.admin-log-eval-dashboard-tutorial（front-end）.md) 告警）。
...
| **RAGAS** | 对「综述类」金标问法跑 [Faithfulness](141.ragas-faithfulness-tutorial.md) |
...
便于 [192 Embedding 成本](192.embedding-batch-cost-tutorial.md) 同类方法做 **LLM 摘要成本** 仪表盘（[183 用量统计](183.admin-usage-dashboard-tutorial（front-end）.md)）。

### 12.9 多语言长文档

中英混排手册 Map 时 **按语言分桶** 再 Reduce，避免模型在 Reduce 时 **翻译漂移**；或统一要求输出 **中文** 但 Map 阶段保留 **英文专有名词** 原文。与 [70 混合语言 Embedding](70.mixed-language-embedding-tutorial.md) 的「分库」思路类似，摘要层也 **勿强行混语言合并**。
...
在 [143 金标回归](143.golden-dataset-tutorial.md) 加 **路由标签** 用例：同一 doc 各 5 条 `global_summary` 与 `factoid`，发布前 **自动验** 路由是否进摘要链/检索链。Map-Reduce 上线后最常见事故是 **路由没改**——事实题也进摘要，Faithfulness **表面仍高**（摘要里自编的数字 **自洽**）但 **与原文不符**。
...
- 能解释为何摘要索引不能替代带页码引用？  
- 能在 [182 调试台](182.retrieval-debug-console-tutorial（front-end）.md) 标 `route=global_summary` 并验金标？
...
4. 写 10 条金标：5 条 `global_summary`、5 条 `factoid`，接 [161 回归](143.golden-dataset-tutorial.md)。  
5. 在 `experiments/map-reduce-poc.md` 记录 **调用次数 × 单价** 与 **Recall@1** 对比扁平 RAG。

### 13.4 路线图 224 验收标准

**运营或开发能在 ingest 流水线触发一次全文摘要任务**，在 [180 进度 UI](180.index-progress-ui-tutorial（front-end）.md) 看到 `map_reduce_done` 日志字段（§12.8），且用户对「全文风险类」问法的内部金标 Recall 优于纯 Top-10 检索——即 224 验收通过。PoC 可夜间批处理；生产须配额（[169 限流](169.rate-limiting-api-tutorial.md)）。
...
**背景**：金融客户年报式审计 PDF，用户问「全文重大缺陷分几类」。**旧方案** Top-10 检索只能覆盖部分章节，模型归纳常漏一类。**新方案** ingest 时 45 段 Map + 9+2 次 Reduce，将 `summary_level=2` 入库；Adaptive 路由 `global_summary` 问法命中摘要 chunk，金标 Recall@1 从 0.55→0.88。**教训**：金额类 factoid 仍强制走向量检索，摘要只服务总览。日志字段见 §12.8，成本 spike 进 [183 用量看板](183.admin-usage-dashboard-tutorial（front-end）.md)。
...
中英混排手册 Map 时 **按语言分桶** 再 Reduce，避免 Reduce 阶段 **翻译漂移**；或统一输出中文但 Map 保留英文专有名词。摘要 chunk ACL 与叶子同级（[53](53.metadata-acl-tutorial.md)）；高度浓缩的摘要 **泄露面更大**（§12.10）。与 [70 混合语言](70.mixed-language-embedding-tutorial.md) 分库思路一致：摘要层也 **勿强行混语言合并**。
