...
**档位：** 主线篇（Part 2）
**本文边界：** 前驱篇 [219](219.tool-schema-design-tutorial.md) Schema 契约。本篇格式/业务/跨字段三层校验。不讲前端表单。接续 [221](221.tool-result-normalization-tutorial.md)。
**动手路径：** ① 读三层校验表 → ② 运行 `demo_validation()` → ③ 对照 §常见失败模式
