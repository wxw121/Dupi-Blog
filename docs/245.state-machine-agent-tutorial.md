...
**档位：** 主线篇（Part 6）
**本文边界：** 前驱篇 [244](244.agent-workflow-patterns-tutorial.md) 编排模式。本篇 FSM 约束。持久化见 [246–247](246.agent-checkpoint-resume-tutorial.md)。接续 [246](246.agent-checkpoint-resume-tutorial.md)。
**动手路径：** ① 读 FSM 转移表 → ② 运行 `demo_fsm()` → ③ 试非法转移
...
对照上图：FSM 约束——246 checkpoint、247 Temporal。
