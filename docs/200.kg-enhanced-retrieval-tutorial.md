...
精排阶段可把 `in_graph: bool` 作为特征（模式 C），在图质量一般时避免硬过滤。整条管道应在 [182 检索调试台](182.retrieval-debug-console-tutorial（front-end）.md) 可复现：`kg_entities`、`kg_doc_whitelist`、`retrieval_mode` 与最终 `chunk_ids` 同屏展示。
...
**日志字段**：`kg_entities`、`kg_doc_whitelist`、`retrieval_mode`、`chunk_ids`——方便与 [182 检索调试台](182.retrieval-debug-console-tutorial（front-end）.md) 对齐。
...
**调试台**（[182](182.retrieval-debug-console-tutorial（front-end）.md)）展示：`kg_entities=["上海项目"]`，`kg_doc_whitelist` 三项，运营可一眼看出 **图是否指错文档**。
...
- [ ] ACL ∩ whitelist 单测覆盖  
- [ ] [182 调试台](182.retrieval-debug-console-tutorial（front-end）.md) 展示 `kg_entities`  
- [ ] 金标桥接集季度回归  
