...
**档位：** 地基篇（Part 1）
**本文边界：** 前驱篇 [201 Agentic RAG](201.agentic-rag-tutorial.md) 与 RAG 系列（检索/生成）。本篇 RAG→Agent 最小演化（Level 0→2）。不讲 LLM API 与生产部署。接续 [215](215.agent-vs-rag-vs-workflow-tutorial.md) 四形态选型。
**动手路径：** ① 读 §最小示例 Level 0→2 → ② `python -m demos.demo_214`（`examples/agent-platform`）→ ③ 对照 §工程化版本

### 核心术语（本篇首次出现）
**RAG**（Retrieval-Augmented Generation）：检索增强生成，先查资料再让模型回答。
通俗说：像开卷考试——允许翻笔记再答题。
**Agent**（智能体）：能根据目标自主多步决策并调用工具的系统。
通俗说：像办事员——你给目标，它自己拆步骤、查资料、动手办。
**工具**（Tool）：Agent 可调用的外部能力（搜索、计算、写库等）。
通俗说：Agent 的「手」，用来碰真实世界。
...
对照上图：Level 0 纯 RAG 到 Level 2 最小 Agent 循环——关键跃迁是「能观察环境并选工具」，不是多检索几次。
...
下面用流程图概括本节代码演示的主线（与 §它解决什么问题 中的能力边界图互补）。
...
对照上图：Level 0→2 演化路线——下一篇 215 在四形态里给选型坐标。
