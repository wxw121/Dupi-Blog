...
**档位：** 主线篇（Part 2）
**本文边界：** 前驱篇 [223](223.idempotent-agent-tools-tutorial.md) 可安全重试。本篇 HITL 审批分级。不讲审批 UI/OA。接续 [225](225.agent-tool-permission-boundary-tutorial.md)。
**动手路径：** ① 读 HITL 分级表 → ② 运行 `demo_hitl()` → ③ 改权限为 BLOCK 观察行为
...
通俗说：工具默认暂停。
**HITL**（Human-in-the-Loop）：高风险或不可逆操作前插入人工审批闸门
通俗说：敏感动作先问人再执行。
---
...
对照上图：HITL 是风险闸门——AUTO/CONFIRM/BLOCK 分级比「全人工」或「全自动」更贴近生产。
...
对照上图：高风险动作先过 HITL 闸门——自动与人工审批要分级，不能全放行或全拦截。
```text
