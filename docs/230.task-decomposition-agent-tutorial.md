...
**档位：** 主线篇（Part 3）
**本文边界：** 前驱篇 [229](229.reflection-agent-pattern-tutorial.md) 质量自检。本篇 Goal/Step/Action 分解。不讲 Multi-Agent。接续 [231](231.agent-stop-condition-tutorial.md)。
**动手路径：** ① 读 Goal/Step 分解 → ② 运行 `demo_decompose()` → ③ 对照 §常见失败模式

### 要不要读
- **建议读：** 用户目标模糊、需拆成可执行子任务时
- **可先跳过：** 单工具单次调用即可完成的请求
- **何时回来读：** Agent 在复杂任务上步骤混乱时
### 核心术语（本篇首次出现）
