...
**档位：** 主线篇（Part 3）
**本文边界：** 前驱篇 [218–225](218.tool-calling-basics-tutorial.md) 工具工程全链。本篇 OTA 循环与 Part 2 继承表。不讲异步细节（见 [222](222.tool-error-timeout-retry-tutorial.md)）。接续 [227](227.react-agent-pattern-tutorial.md)。
**动手路径：** ① 读 Part 2 继承表 → ② 运行 `demo_ota_loop()` → ③ 对照 §工程化版本
...
对照上图：OTA 循环是 Part 3 主轴——Observe 收反馈，Think 选工具，Act 调 Part 2 能力链。
...
对照上图：Observe→Think→Act 循环把 Part 2 工具链串进 ACT——没有停止条件会无限转圈（见 231）。
```text
为什么需要显式的 Agent 循环？
...
对照上图：OTA 与 Part 2 继承表——227 起进 ReAct 等模式。
