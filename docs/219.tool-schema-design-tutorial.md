...
**档位：** 主线篇（Part 2）
**本文边界：** 前驱篇 [218](218.tool-calling-basics-tutorial.md) 注册表与执行器。本篇 Schema 三读者与 Pydantic。不讲各云 function 差异。接续 [220](220.tool-parameter-validation-tutorial.md)。
**动手路径：** ① 读 Schema 三读者 → ② 运行文末 `demo_schema()` → ③ 故意改错字段观察校验
...
对照上图：Schema 三读者——字段变更要同时想模型、校验器与文档。
