...
**档位：** 主线篇（Part 6）
**本文边界：** 前驱篇 [215](215.agent-vs-rag-vs-workflow-tutorial.md) Workflow + [243](243.rag-agent-bad-case-debugging-tutorial.md) 质量收束。本篇七种编排模式。代码 `main.py 244`。接续 [245](245.state-machine-agent-tutorial.md)。
**动手路径：** ① `python main.py 244` → ② 对照 §七种模式决策表 → ③ 为场景选一种模式
...
通俗说：ReAct/FSM/事件驱动等组织 Agent 步骤的方。
---
...
对照上图：七种模式——245 FSM、246 检查点进可靠性线。
