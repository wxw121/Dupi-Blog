...
4. 做 **重索引 / 换模型** 预算乘数（§7）。  
5. 在 [FastAPI 项目结构](156.fastapi-project-structure-tutorial.md) 立项表填 **索引预算行**。
...
清洗不足（[46 清洗](46.text-cleaning-tutorial.md)）——无意义短块仍占 N。
...
立项时不要只写「预计很便宜」——工程、财务、产品应讨论同一组变量。模板复制到 Wiki 后，每次大批量入库或换模型前更新一行「重索引乘数」与「缓冲 %」，评审会上可直接对照 [184 看板](184.admin-log-eval-dashboard-tutorial（front-end）.md) 的 embed 费用卡片。
