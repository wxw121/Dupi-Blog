...
**档位：** 收束篇（Part 6）
**本文边界：** 前驱篇 [246](246.agent-checkpoint-resume-tutorial.md) 手动检查点。本篇 Temporal/持久执行。`temporal_minimal.py`。接续 [248](248.agent-background-job-tutorial.md)。
**动手路径：** ① 读 246 checkpoint → ② 对照 §对照前文 Temporal 表 → ③ `python temporal_minimal.py`（需 Docker）
...
通俗说：编排逻辑本身，进程死了也能续。
**Temporal**（Temporal）：托管长流程、重试与人工等待的持久执行平台
通俗说：进程死了任务还能续跑的平台。
---
...
对照上图：Durable Execution 让「进程死了任务不死」——长等待与重试由平台托管。
...
对照上图：长等待与进程重启需要 Durable Execution——Activity 副作用与可重放逻辑要分离。
### 长任务的挑战
...
对照上图：Durable 语义——248 后台、249 事件驱动。
